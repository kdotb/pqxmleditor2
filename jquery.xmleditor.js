...
$.widget( "xml.xmlEditor", {
	options: {
		...
		documentTitle : null,
		addTopMenuHeaderText : 'Top Element list',
		addAttrMenuHeaderText : 'Attribute list',
		addElementMenuHeaderText : 'Subelement list',
	...
	this.addProblem('this feature will be added soon');
	},
	
	openXML: function(node) {
	//alert('Coming soon');
	//this.addProblem('this feature will be added soon');
   ...
   node.dispatchEvent(evt);
	...
	//this.activeEditor.activate();
		...
		//alert (xmlString);
		$("." + submissionStatusClass).html("Submitting...").css("background-color", "#ffbbbb").animate({backgroundColor: "#fffffd"}, 1000);
		
		var self = this;
		//alert (this.options.ajaxOptions.xmlUploadPath);
		$.ajax({
			async: true,
			...
			beforeSend : function(xhr){
				$("." + submissionStatusClass).html("Sending now...").css("background-color", "#ffbbbb").animate({backgroundColor: "#fffffd"}, 1000);
			},
			...
			complete : function(xhr,status){
				$("." + submissionStatusClass).html("Complete saving").css("background-color", "#ffbbbb").animate({backgroundColor: "#fffffd"}, 1000);
			}
		});
	},
	...
	swordSubmitResponseHandler: function(response) {
		...
		//alert(localName(responseObject[responseObject.length - 1]));  ---------------->"/RECORD"
...
AbstractXMLObject.prototype.createElementInput = function (inputID, startingValue, appendTarget){
	...
	if (this.objectType.values.length > 0){
		...
		//$(input).autosize();
		var self = this;
		//alert(self.objectType.attribute);
		//alert(self.editor.options.expandingTextAreas);
...
DocumentState.prototype.updateStateMessage = function () {
	if (this.isChanged()) {
			$("." + submissionStatusClass).html("Unsaved changes").css("background-color", "#ffbbbb").animate({backgroundColor: "#fffffd"}, 1000);
	} else {
			$("." + submissionStatusClass).html("All changes saved").css("background-color", "#ffbbbb").animate({backgroundColor: "#fffffd"}, 1000);
	}
};
	...
	this.headerMenuData = [ {
		...
		items : [ 
			/*{
				...
				binding : "alt+shift+o",
				action : $.proxy(self.editor.openXML, self.editor)
			},*/
			{
				label : 'Validate XML',
				enabled : (self.editor.options.ajaxOptions.xmlUploadPath != null),
				binding : "alt+shift+v",
				action : $.proxy(self.editor.validateXML, self.editor)
			},
			{
				label : 'Submit to EBD Server',
				enabled : (self.editor.options.ajaxOptions.xmlUploadPath != null),
				binding : "alt+shift+s",
